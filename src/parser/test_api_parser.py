...
"""
Test du parser d'articles Medium avec l'API

Ce script permet de tester le parser d'articles Medium basé sur l'API non officielle.
"""
...
from parser.medium_api_parser import MediumAPIParser

def main():
    """Fonction principale pour tester le parser."""
    
    # Créer un répertoire de sortie pour les tests
    output_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'output', 'test_api_parser')
    ...
    print(f"Test du parser API avec l'article: {test_url}")
    print(f"Sortie dans: {output_dir}")
    
    # Initialiser le parser
    parser = MediumAPIParser(output_dir=output_dir)
