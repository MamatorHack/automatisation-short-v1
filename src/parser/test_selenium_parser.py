...
"""
Test du parser d'articles Medium avec Selenium

Ce script permet de tester le parser d'articles Medium basé sur Selenium.
"""
...
from parser.medium_selenium_parser import MediumSeleniumParser

def main():
    """Fonction principale pour tester le parser."""
    
    # Créer un répertoire de sortie pour les tests
    output_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'output', 'test_selenium_parser')
    ...
    print(f"Test du parser Selenium avec l'article: {test_url}")
    print(f"Sortie dans: {output_dir}")
    
    # Initialiser le parser
    parser = MediumSeleniumParser(output_dir=output_dir, headless=True)
