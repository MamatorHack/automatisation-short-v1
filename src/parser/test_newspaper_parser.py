...
"""
Test du parser d'articles Medium avec newspaper3k

Ce script permet de tester le parser d'articles Medium basé sur newspaper3k.
"""
...
from parser.medium_newspaper_parser import MediumNewspaperParser

def main():
    """Fonction principale pour tester le parser."""
    
    # Créer un répertoire de sortie pour les tests
    output_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'output', 'test_newspaper_parser')
    ...
    print(f"Test du parser newspaper3k avec l'article: {test_url}")
    print(f"Sortie dans: {output_dir}")
    
    # Initialiser le parser
    parser = MediumNewspaperParser(output_dir=output_dir)
