...
"""
Test du parser d'articles Medium (Version simplifiée) avec une URL alternative

Ce script permet de tester le parser d'articles Medium basé sur une approche hybride JSON/BeautifulSoup.
"""
...
def main():
    ...
    output_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'output', 'test_simple_parser')
    os.makedirs(output_dir, exist_ok=True)
    
    # URL alternative d'un article Medium
    test_url = "https://medium.com/@daltonmabery/10-extraordinary-productivity-hacks-from-the-worlds-most-successful-people-d850d47ddb0"
    
    print(f"Test du parser simplifié avec l'article: {test_url}")
    print(f"Sortie dans: {output_dir}")
    
    # Initialiser le parser
    parser = MediumSimpleParser(output_dir=output_dir)
