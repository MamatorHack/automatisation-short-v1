...
"""
Test du parser d'articles Medium

Ce script permet de tester le parser d'articles Medium avec un exemple d'article.
"""
...
from parser.medium_parser import MediumParser

def main():
    """Fonction principale pour tester le parser."""
    
    # Créer un répertoire de sortie pour les tests
    output_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'output', 'test_parser')
    ...
    print(f"Test du parser avec l'article: {test_url}")
    print(f"Sortie dans: {output_dir}")
    
    # Initialiser le parser
    parser = MediumParser(output_dir=output_dir)
